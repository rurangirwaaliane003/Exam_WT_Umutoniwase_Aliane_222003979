...
<section>
    ...
    <h1><u> eventsessions Form </u></h1>
    <form method="post" onsubmit="return confirmInsert();">
            
        <label for="sid">session_id:</label>
        ...
        <input type="text" id="stt" name="stt" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO eventsessions(session_id, event_id, session_title) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $sid, $eid, $stt);
...
// SQL query to fetch data from the eventsessions table
$sql = "SELECT * FROM eventsessions";
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of eventsessions</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of eventsessions</h2></center>
    <table border="5">
        <tr>
            <th>session_id</th>
            <th>event_id</th>
      ...
      include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all eventsessions
        $sql = "SELECT * FROM eventsessions";
        $result = $connection->query($sql);

        // Check if there are any eventsessions
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $sid = $row['session_id']; // Fetch the session_id
                echo "<tr>
                    <td>" . $row['session_id'] . "</td>
                    <td>" . $row['event_id'] . "</td>
                    <td>" . $row['session_title'] . "</td>
                    <td><a style='padding:4px' href='delete_eventsessions.php?session_id=$sid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_eventsessions.php?session_id=$sid'>Update</a></td> 
                </tr>";
            }
        } else {
