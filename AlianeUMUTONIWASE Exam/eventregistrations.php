...
<head>
  ...
  <title>eventregistrations</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
    ...
    <h1><u> eventregistrations Form </u></h1>
    <form method="post" onsubmit="return confirmInsert();">
            
        <label for="rid">registration_id:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO eventregistrations(registration_id, id, event_id) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $rid, $uid, $eid);
...
// SQL query to fetch data from the eventregistrations table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of eventregistrations</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of eventregistrations</h2></center>
    <table border="5">
        <tr>
     ...
     include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all eventregistrations
        $sql = "SELECT * FROM eventregistrations";
        $result = $connection->query($sql);

        // Check if there are any eventregistrations
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $rid = $row['registration_id']; // Fetch the registration_id
                echo "<tr>
                    <td>" . $row['registration_id'] . "</td>
                    <td>" . $row['id'] . "</td>
                    <td>" . $row['event_id'] . "</td>
              
                    <td><a style='padding:4px' href='delete_eventregistrations.php?CustomerID=$rid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_eventregistrations.php?CustomerID=$rid'>Update</a></td> 
                </tr>";
            }
        } else {
...
<footer>
  <center> 
    ...
    <b><h2>UR CBE BIT &copy, 2024 &reg, Designer by: @Aliane UMUTOIWASE</h2></b>
  </marquee>
  </center>
</footer>
</body>
