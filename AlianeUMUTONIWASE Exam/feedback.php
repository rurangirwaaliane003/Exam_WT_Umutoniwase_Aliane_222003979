...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO feedback(feedback_id, id, event_id) VALUES (?, ?, ?)");
    $stmt->bind_param("iii", $fid, $uid, $eid);
     ...
     include('database_connection.php');
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $fid = $row['feedback_id']; // Fetch the feedback_id
                echo "<tr>
                    <td>" . $row['feedback_id'] . "</td>
                    <td>" . $row['id'] . "</td>
                    <td>" . $row['event_id'] . "</td>
                    <td><a style='padding:4px' href='delete_feedback.php?feedback_id=$fid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_feedback.php?feedback_id=$fid'>Update</a></td> 
                </tr>";
            }
        } else {
