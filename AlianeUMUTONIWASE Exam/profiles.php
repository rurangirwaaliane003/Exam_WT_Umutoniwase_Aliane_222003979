...
<section>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <label for="pid">profile_id:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO profiles(profile_id,id, bio) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $pid, $uid, $bo);
...
// SQL query to fetch data from the profiles table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of profiles</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of profiles</h2></center>
    <table border="5">
        <tr>
            <th>profile_id</th>
            <th>id</th>
      ...
      include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all profiles
        $sql = "SELECT * FROM profiles";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $pid = $row['profile_id']; // Fetch the profile_id
                echo "<tr>
                    <td>" . $row['profile_id'] . "</td>
                    <td>" . $row['id'] . "</td>
                    <td>" . $row['bio'] . "</td>
                    <td><a style='padding:4px' href='delete_profiles.php?profile_id=$pid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_profiles.php?profile_id=$pid'>Update</a></td> 
                </tr>";
            }
        } else {
